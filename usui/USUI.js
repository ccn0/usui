...
const USUI = {
        ...
        __autoAttribute__: (disallowList = [], paramsObj, element) => {
            Object.entries(paramsObj ?? {}).forEach(([key, val]) => {
                ...
                switch (key) {
                    ...
                    case "listeners":
                        if (typeof val === "object") {
                            Object.entries(val).forEach(([eType, eFunc]) => {
                                element.addEventListener(eType, eFunc);
                            });
                        };
                        break;
                    default:
    ...
    createPopup: (params = {
        ...
        if ("buttons" in params) params.buttons.forEach((button) => {
            if ("type" in button) {
                ...
                tbInput.classList.add("USUI_titlebarinput");
                tbInput.type = button.type || "text";
                if (!(["color","file","range"]).includes(tbInput.type)) tbInput.classList.add("USUI_input");
                if (button.classes) {
                    tbInput.classList.add(...button.classes);
                };
                tbInput.addEventListener("mousedown", (event) => {
                    event.stopPropagation();
                });
                USUI.lib.__autoAttribute__(["type","classes"],button,tbInput);
                popupTitlebar.appendChild(tbInput);
            } else {
    ...
    modules: {
        ...
        BBinput: (params = {
            ...
            if (!(["color","file","range"]).includes(input.type)) input.classList.add("USUI_input");
